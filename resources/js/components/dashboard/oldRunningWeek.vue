<template>
    <div>
        <div class="schedule_main_all _b_color">
            <div class="_profile_card_all schedule_all" v-if="list.length" >
                <!-- card -->
                <div class="schedule_items  _color _dis_flex _box_shadow  _bg" v-for="(item,index) in list " :key="index" >
...
export default {
    ...
    methods:{
        async getWorkingEventList(){
            const res  = await  this.callApi('get',`getAllOldEventList`)
            if(res.status===200){
                this.list = res.data
            }
            else{
